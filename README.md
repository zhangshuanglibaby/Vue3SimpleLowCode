...
- simple-admin的编辑页面
  ...
  - 开发simple-admin 项目 配置区 视口切换组件
    - 在前台侧，如果某些组件仅桌面端展示，那么在移动端时则用display:none就可以控制组件的显示和隐藏
    - 在后台编辑侧，如果某些组件仅桌面端展示，那么在切换移动端是需要有个 蒙版标识
    - 基于以上就有个问题，前台和后台对于组件的渲染 需要不同的逻辑，我们需要给simple-ui组件库传递标识
    - app.use 注册插件时，可以传递一些额外的参数给simple-ui组件库
    - 在simple-ui接收一些 平台的标识参数，使用provide提供给所有的子组件使用
  - 开发simple-admin 项目 渲染区 悬浮操作栏
    - 注意 在嵌套容器中 里面的组件也是可以复制的
  - 表单的验证
